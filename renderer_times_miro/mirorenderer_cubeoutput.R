mirorenderer_cubeoutputOutput <- function(id, height = NULL, options = NULL, path = NULL){
  ...
  tagList(
    ...
    tags$div(class = "custom-css",
             tabsetPanel(id = ns("outputTabs"),
                         ...
                         tabPanel(class = "default-height",
                                  textOutput(ns("dataExplorerPivotTabName")),
                                  value = "dataExplorerPivot",
                                  miroPivotOutput(ns("customMiroPivot"), options = list(
                                    "_metadata_" = list(symname = "explorer_df",
                                                        headers = list(
                                                          Sector = list(
                                                            type = "string",
                                                            alias = "Sector"
                                                          ...
                                                          Technology_Group = list(
                                                            type = "string",
                                                            alias = "Technology_Group"
                                                          ),
                                                          Value = list(
                                                            type = "numeric",
                                                            alias = "Value"
                         ...
                         tabPanel(class = "default-height", style = "margin-top: 5px;",
                                  
                                  textOutput(ns("fullOutputTabName")),
                                  value = "fullOutput",
                                  miroPivotOutput(ns("fullOutput"), options = list(
                                    "_metadata_" = list(symname = "data",
                                                        headers = list(
                                                          Attribute = list(
                                                            type = "string",
                                                            alias = "Attribute"
                                    ...
                                    "_input_" = FALSE,
                                    domainFilter = list(
                                      domains = c("Region", "Attribute", "Commodity", "Period", "Process", "TimeSlice", "Vintage", "UserConstraint"),
                                      filterVal = "-",
                                      showAll = TRUE
                                    )
                                  ))
...
renderMirorenderer_cubeoutput <- function(input, output, session, data, options = NULL, path = NULL, rendererEnv = NULL, views = NULL, outputScalarsFull = NULL, ...){
  ns <- session$ns
  ...
  output$dataExplorerPivotTabName <- renderText({generalConfig$tabNames$dataExplorerPivot})
  output$fullOutputTabName <- renderText({generalConfig$tabNames$fullOutput})
  ...
  if (length(dataProcessing$excludeColumns)){
    for (exclude in dataProcessing$excludeColumns) {
      if(exclude %in% colnames(data))
        data <- data %>% select(-all_of(exclude))
    }
  }
  ...
  if (length(dataProcessing$columnNames)){
    if(length(dataProcessing$columnNames) == length(colnames(data))){
      colnames(data) <- dataProcessing$columnNames
    } else {
      abortSafe(
        sprintf(
          "Number of column dimensions provided in dataProcessing$columnNames does not match the number of columns in the data. Current data colum names are '%s'",
          colnames(data)
        ))
    }
  }
  ...
  dataExplorer <- config$dataExplorer
  
  # read and execute data explorer code
  if(length(dataExplorer)){
    # Import schema
    ...
    clean_df <- data %>%  
      ...
      inner_join(schema_all, data, by = c("Attribute", "Process", "Commodity"), relationship = "many-to-many") %>%
    ...
    # Set Emission values to zero for non emission fuel
    if(length(dataExplorer$nonEmissionFuel) && 
       !is.null(dataExplorer$nonEmissionFuel$fuels) && 
       length(dataExplorer$nonEmissionFuel$fuels)){
      clean_df <- clean_df %>%
        mutate(Value = ifelse(Fuel %in% dataExplorer$nonEmissionFuel$fuels &  Parameters == dataExplorer$nonEmissionFuel$emissionParameter, 0,Value) )
    }
    
    # complete data for all periods by adding zeros
    clean_df <- clean_df %>%
      complete(Period,nesting(Sector, Subsector, Technology, Enduse, Unit, Parameters, Fuel, FuelGroup, Technology_Group),fill = list(Value = 0))
    ...
    if(length(dataExplorer$preprocessing)){
      externalCode <- readLines(file.path(customRendererDir, "data-processing", dataExplorer$preprocessing), warn = FALSE)
    ...
    if(!identical(colnames(explorer_df), 
                  c("Sector", "Subsector", "Technology", "Enduse", "Unit",
                    ...
                    "Technology_Group", "Value"))) {
      abortSafe("wrong column names for tibble explorer_df! Must be c('Sector', 'Subsector', 'Technology', 'Enduse', 'Unit', 'Parameters', 'Fuel', 'Period', 'FuelGroup', 'Technology_Group', 'Value')")
    }
      ...
      tabs <- lapply(names(dataExplorer$tabNames), function(tabName) {
        ...
        tabPanel(
          ...
          fluidRow(style = "margin-top: 15px",
                   column(12,
                          tags$div(id = ns(paste0(id, "TableWrapper")), style = "display:none;",
                                   tags$h3(class = "custom-table-header", 
                                           textOutput(ns(paste0(id, "tableHeader")))),
                                   dataTableOutput(ns(paste0(id, "Table")))
                          ),
                          tags$div(class = "dashboard-chart-wrapper", 
    ...
    lapply(names(dataExplorer$tabNames), function(tabName) {
      ...
      observeEvent(input$chart_type, {
        if (identical(input$chart_type, "table")) {
          hideEl(session, paste0("#", session$ns(paste0(id, "Chart"))))
          showEl(session, paste0("#", session$ns(paste0(id, "TableWrapper"))))
        } else {
          hideEl(session, paste0("#", session$ns(paste0(id, "TableWrapper"))))
          showEl(session, paste0("#", session$ns(paste0(id, "Chart"))))
        }
      })
      
      output[[paste0(id, "tableHeader")]] <- renderText({
        title <- rv$tableHeader
        title
      })
      ...
      output[[paste0(id, "Chart")]] <- chartjs::renderChartjs({
    ...
    output$downloadButtons <- renderUI({
      ...
      if (is.null(input$tabs) || identical(input$tabs, "All Sectors")) {
        id <- "overview"
      } else {
        id <- tolower(input$tabs)
      }
      
      canvasId <- paste0(id, "Chart")
      fileName <- idMap[[id]]
  ...
  if (length(dashboard)) {
    ...
    lapply(1:length(dashboard$valueBoxes$Id), function(i) {
      ...
      output[[valBoxName]] <- renderValueBox({
        ...
        if(!is.null(valueTmp)) {
          valueTmp <- valueTmp %>% pull(value) %>% as.numeric()
          if(!is.na(dashboard$valueBoxes$Decimals[i])) {
            valueTmp <- valueTmp %>% round(as.numeric(dashboard$valueBoxes$Decimals[i]))
          }
        }
    ...
    # Data views
    # Distinguishes between dashboard$dataViews objects with one and multiple elements 
    # names(dashboard$dataViews) must match dashboard$valueBoxes$Id entries
  ...
  if(length(dataExplorer) && length(generalConfig$tabNames$dataExplorerPivot)){
    renderMiroPivot("customMiroPivot", explorer_df, 
                    options = list(
                      "_metadata_" = list(symname = "explorer_df",
                                          headers = list(
                                            Sector = list(
    ...
    hideTab("outputTabs", "dataExplorerPivot", session = session)
  }
  
  if(length(generalConfig$tabNames$fullOutput)){
    renderMiroPivot("fullOutput", data, 
                    options = list(
                      "_metadata_" = list(symname = "data",
                                          headers = list(
                                            Attribute = list(
    ...
    hideTab("outputTabs", "fullOutput", session = session)
  }
